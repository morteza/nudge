...
public class Global extends GlobalSettings {
	...
	public void onStart(Application app) {
	    ...
	    if(Account.find.all().size()==0){
	        Account admin = new Account("Administrator", "admin@onto.ir", "hello123");
	        admin.role= AccountRole.ADMIN;
	        admin.isConfirmed=true;
	        admin.save();
	    }
	    Logger.info("$onto.nudge.title$ is starting... (Accounts: "+ Account.find.all().size() + ")");
	}
	
	@Override
	public void onStop(Application app) {
		Logger.info("$onto.nudge.title$ is stopping...");
	}
