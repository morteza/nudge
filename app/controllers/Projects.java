...
public class Projects extends Controller {
    ...
    public static Result board(Long id) {
        ...
        //FIXME fetch tasks that belongs to the passed id argument
        List<Task> proposals = Task.find.where().eq("state", TaskState.PROPOSED).findList();
        List<Task> todos = Task.find.where().eq("state", TaskState.TODO).findList();
        List<Task> workings = Task.find.where().eq("state", TaskState.IN_PROGRESS).findList();
        List<Task> dones = Task.find.where().eq("state", TaskState.DONE).findList();
